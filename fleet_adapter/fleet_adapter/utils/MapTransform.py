...
class MapTransform:
    ...
    def robot_to_rmf_meters(self, robot_coord: LionsbotCoord) -> RmfCoord:
        ...
        rmf_coord_reference_frame = self._robot_to_rmf_reference_meters(robot_coord)
        ...
        if robot_coord.orientation_radians is None:
            ...
            return RmfCoord(
                x=rmf_coord.x, 
                y=rmf_coord.y, 
                orientation_radians=self.wrap_orientation(robot_coord.orientation_radians - self._get_orientation_offset()))
        
    def robot_to_rmf_pixels(self, robot_coord: LionsbotCoord) -> RmfCoord:
        """
            ...
            As coordinate frame is +X=right and +Y=down in RMF traffic editor, we flip the y-axis
        """
        rmf_coord_meters = self._robot_to_rmf_reference_meters(robot_coord)
        return RmfCoord(
            x=rmf_coord_meters.x / self.floorplan_scale_meters_per_pixel,
            y=(-rmf_coord_meters.y / self.floorplan_scale_meters_per_pixel))
    ...
    def rmf_meters_to_robot(self, rmf_coord: RmfCoord) -> LionsbotCoord:
        ...
        if rmf_coord.orientation_radians is None:
            ...
            return LionsbotCoord(
                x=robot_x, 
                y=-robot_y, 
                orientation_radians=self.wrap_orientation(rmf_coord.orientation_radians + self._get_orientation_offset()))

    def _robot_to_rmf_reference_meters(self, robot_coord: LionsbotCoord) -> RmfCoord:
        rmf_x, rmf_y = self.robot_to_rmf_meters_transform.transform(
            [robot_coord.x, -robot_coord.y])
    ...
    @staticmethod
    def wrap_orientation(orientation_radians:float):
        return (orientation_radians + math.pi) % (2 * math.pi) - math.pi
